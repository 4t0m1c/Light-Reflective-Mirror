...
namespace LightReflectiveMirror {
    ...
    public partial class LightReflectiveMirrorTransport : Transport {
        ...
        private void Connect (string serverIP, ushort port = 7777) {
            ...
            // Increase buffer size to accommodate new data
            ...
            try {
                ...
                switch (opcode) {
                    ...
                    case OpCodes.RoomCreated:
                        try {
                            Debug.Log ($"LRM Debug: Processing RoomCreated. Data length: {data.Length}, Current pos: {pos}");

                            // Log the next few bytes to see what we're trying to read
                            string bytesDebug = "";
                            for (int i = pos; i < Math.Min (pos + 10, data.Length); i++) {
                                bytesDebug += $"{data[i]:X2} ";
                            }

                            Debug.Log ($"LRM Debug: Next bytes: {bytesDebug}");

                            serverId = data.ReadString (ref pos);
                            Debug.Log ($"LRM Debug: Read serverId: {serverId}");

                            groupId = data.ReadString (ref pos);
                            Debug.Log ($"LRM Debug: Read groupId: {groupId}");

                            authorityLevel = data.ReadInt (ref pos);
                            Debug.Log ($"LRM Debug: Read authorityLevel: {authorityLevel}");

                            Debug.Log ($"LRM Debug: Room created with ID: {serverId}, Group: {groupId}, Auth: {authorityLevel}");
                        } catch (Exception e) {
                            Debug.LogError ($"LRM Error in RoomCreated: {e.Message}\nStack: {e.StackTrace}\nCurrent position: {pos}");
                            throw;
                        }
                    ...
                    case OpCodes.ServerListResponse:
                        try {
                            ...
                            Debug.LogError ($"LRM: Error processing server list: {e.Message}");
                        }
        ...
        public void UpdateRoomGroupId (string newGroupId) {
            if (IsServer) {
                int pos = 0;
                _clientSendBuffer.WriteByte (ref pos, (byte) OpCodes.UpdateRoomData);
                ...
                _clientSendBuffer.WriteBool (ref pos, false);
                _clientSendBuffer.WriteBool (ref pos, true);
                _clientSendBuffer.WriteString (ref pos, newGroupId);

                clientToServerTransport.ClientSend (new ArraySegment<byte> (_clientSendBuffer, 0, pos), 0);
            }
        }

        public void UpdateRoomAuthorityLevel (int newAuthorityLevel) {
            if (IsServer) {
                int pos = 0;
                _clientSendBuffer.WriteByte (ref pos, (byte) OpCodes.UpdateRoomData);
                ...
                _clientSendBuffer.WriteBool (ref pos, false);
                _clientSendBuffer.WriteBool (ref pos, true);
                _clientSendBuffer.WriteInt (ref pos, newAuthorityLevel);
        ...
        public enum OpCodes {
            ...
            DirectConnectIP = 21,
            RequestServerList = 24, // New OpCode for filtered server list request
            ServerListResponse = 25 // New OpCode for server list response
        }
    ...
    public struct Room {
        ...
        public string groupId; // New: Group identifier for filtering
        public int authorityLevel; // New: Authority level for access control
    }
