...
namespace LightReflectiveMirror.Endpoints {
    ...
    public class Endpoint {
        ...
        private static List<Room> FilterRooms (string groupId, int authorityLevel) {
            return Program.instance.GetRooms ()
                .Where (x => x.isPublic &&
                             x.groupId == groupId &&
                             x.authorityLevel <= authorityLevel) // User can see rooms at or below their authority level
                .ToList ();
        }

        [RestRoute ("Get", "/servers/{groupId}/{authLevel:num}")]
        public async Task FilteredServerList (IHttpContext context) {
            if (!Program.conf.EndpointServerList) {
                await context.Response.SendResponseAsync (HttpStatusCode.Forbidden);
                return;
            }

            string groupId = context.Request.PathParameters["groupId"];
            int authLevel = int.Parse (context.Request.PathParameters["authLevel"]);

            var filteredRooms = FilterRooms (groupId, authLevel);
            string json = JsonConvert.SerializeObject (filteredRooms, Formatting.Indented);

            await context.Response.SendResponseAsync (json);
        }

        [RestRoute ("Get", "/compressed/servers/{groupId}/{authLevel:num}")]
        public async Task FilteredServerListCompressed (IHttpContext context) {
            if (!Program.conf.EndpointServerList) {
                await context.Response.SendResponseAsync (HttpStatusCode.Forbidden);
                return;
            }
        ...
        // Add CORS headers for the new endpoints
        [RestRoute ("Options", "/servers/{groupId}/{authLevel:num}")]
        [RestRoute ("Options", "/compressed/servers/{groupId}/{authLevel:num}")]
        public async Task FilteredServerListOptions (IHttpContext context) {
            context.Response.Headers.Add ("Access-Control-Allow-Origin", "*");
            context.Response.Headers.Add ("Access-Control-Allow-Headers", "Accept, X-Access-Token, X-Application-Name, X-Request-Sent-Time");
